...
```
```{r, summary steface_quad_100}
print("summary for quad 100")
m_size
m_code
```

```{r,  plot steface_quad_100}
par(mfrow = c(1, 2))
v_dev <- seq(0, 1, length.out = 5)
plot(v_dev, c(0.05, m_size[1, ]), ylim = c(0, 1), type = "o")
lines(v_dev, c(0.05, m_code[1, ]), ylim = c(0, 1))
plot(v_dev, c(0.05, m_size[2, ]), ylim = c(0, 1), type = "o")
lines(v_dev, c(0.05, m_code[2, ]), ylim = c(0, 1))
```
...
```{r, steface_trig_1007, include=F}
load("steface_trig_1007.RData") # simulation/rmd/
for (i in 1:4) {
    m_size[2, i] <- steface_trig_1007[[i]][[1]]
}
```
```{r, summary steface_trig_100}
print("summary for steface_trig")
m_size
```

```{r,  plot steface_trig_100}
par(mfrow = c(1, 2), ylim = c(0, 1))
v_dev <- seq(0, 1, length.out = 5)
plot(v_dev, c(0.05, m_size[1, ]), ylim = c(0, 1))
plot(v_dev, c(0.05, m_size[2, ]), ylim = c(0, 1))
```
